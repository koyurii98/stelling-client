...
const Mypage = props => {
  ...
  const addOrUpdate = useCallback(async()=>{
    if(!myInfo.name){
      openAlert("이름이 입력되지 않았습니다");
      return;
    }else{
      const token = window.localStorage.getItem("stelling");
      const { res, err } = await requestPut(`${SERVER_URL}user/`,{
        name:myInfo.name
      }, dispatchLoadMask, token);
      if(err){
  ...
  return(
    <Modal className="modal-box">
        ...
        <ModalBody>
            {
              ...
              <div className="profile-first">
                <p><span>STELLING</span> 에 오신것을 환영합니다!&#128154;</p>
                <span>나의 프로필을 등록해보세요!</span>
              </div>
            }
            <div className="profile-img">
              <img src={ user.img ? user.img : "./img/user.png" }></img>
            </div>
        ...
        <ModalFooter>
          {
            modal.edit &&
              <button className="MA-Btn Btn-color-gray" onClick={closeModal}>취소</button>
          }
            <button className="MA-Btn Btn-color-green" onClick={addOrUpdate}>저장</button>
        </ModalFooter>
    </Modal>
  )
}
